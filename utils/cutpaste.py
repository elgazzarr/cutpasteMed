...
class CutPasteNormal(CutPaste):
    ...
    """
    def __init__(self, area_ratio=[0.001,0.01], aspect_ratio=0.3, **kwags):
    ...
    def __call__(self, img):
        ...
        cut_d = int(random.uniform(0.2, 2) * abs(cut_w - cut_h + 2))

        #print(cut_w, cut_h, cut_d)

        # one might also want to sample from other images. currently we only sample from the image itself
        from_location_h = int(random.uniform(30, h- 30 - cut_h))
        from_location_w = int(random.uniform(30, w - 30-  cut_w))
        from_location_d = int(random.uniform(30, d - 30 -cut_d))

        patch = img[from_location_h:from_location_h + cut_h, from_location_w:from_location_w + cut_w, from_location_d:from_location_d + cut_d].copy()

        to_location_h = int(random.uniform(0, h - cut_h))
        to_location_w = int(random.uniform(0, w - cut_w))
        to_location_d = int(random.uniform(0, d - cut_d))

        augmented = img.copy()
        augmented[to_location_h:to_location_h + cut_h, to_location_w:to_location_w + cut_w, to_location_d:to_location_d + cut_d] = patch
        print('Copying and Pasting')
        return augmented
